...
void Canvas::show_background(bool flag) { show_background_ = flag; }
...
void Canvas::clear_shape_list() { shape_list_.clear(); }
...
void Canvas::draw_background() {
  ...
  ImGui::InvisibleButton(label_.c_str(), canvas_size_,
...
void Canvas::draw_shapes() {
  ...
  if (draw_status_ && current_shape_) {
    if (shape_type_ == kPolygon) {
      ...
      polygon->draw_preview(s, mouse_pos_in_canvas());
    } else {
...
void Canvas::mouse_click_event() {
  ...
  if (!draw_status_) {
    ...
    case USTC_CG::Canvas::kLine: {
      current_shape_ = std::make_shared<Line>(start_point_.x, start_point_.y,
                                              end_point_.x, end_point_.y);
      break;
    }
    case USTC_CG::Canvas::kRect: {
      current_shape_ = std::make_shared<Rect>(start_point_.x, start_point_.y,
                                              end_point_.x, end_point_.y);
      break;
    }
    case USTC_CG::Canvas::kEllipse: {
      current_shape_ = std::make_shared<Ellipse>(
          start_point_.x, start_point_.y, fabsf(start_point_.x - end_point_.x),
          fabsf(start_point_.y - end_point_.y));
      break;
    }
    case USTC_CG::Canvas::kPolygon: {
      current_shape_ =
          std::make_shared<Polygon>(start_point_.x, start_point_.y);
      break;
    }
    default:
      break;
...
void Canvas::mouse_move_event() {
  // HW1_TODO: Drawing rule for more primitives
  if (draw_status_ && shape_type_ != Canvas::kPolygon) {
    end_point_ = mouse_pos_in_canvas();
    if (current_shape_) {
      current_shape_->update(end_point_.x, end_point_.y);
    }
...
ImVec2 Canvas::mouse_pos_in_canvas() const {
  ...
  const ImVec2 mouse_pos_in_canvas(io.MousePos.x - canvas_min_.x,
                                   io.MousePos.y - canvas_min_.y);
